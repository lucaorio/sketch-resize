...
module.exports = "file://" + context.plugin.urlForResourceNamed("_webpack_resources/43aa738f582ce37c61cb81dcdf421b63.html").path();
...
var resize = function resize(selection, dimensions) {
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = selection.layers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var layer = _step.value;
      var frame = layer.frame;
      var finalDimensions = process(dimensions, frame.width, frame.height);
  ...
  sketch__WEBPACK_IMPORTED_MODULE_0___default.a.UI.message("Resized ".concat(selection.length, " layer(s)"));
}; // process dimensions, and replace %w, and %h


var process = function process(dimensions, originalWidth, originalHeight) {
  return {
...
var sanitize = function sanitize(string, w, h) {
  return string.replace(/%w/gi, w).replace(/%h/gi, h);
};
...
/* harmony import */ var _actions_resize__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./actions/resize */ "./src/actions/resize.js");
...
var main = function main() {
  ...
  var selection = document.selectedLayers; // return error message if selection is empty

  if (selection.isEmpty) {
    sketch__WEBPACK_IMPORTED_MODULE_1___default.a.UI.message('No layer(s) selected!');
    return;
  ...
  var webview = new sketch_module_web_view__WEBPACK_IMPORTED_MODULE_0___default.a(options);
  var webcontent = webview.webContents; // when loaded, show the webview

  webview.once('ready-to-show', function () {
    return webview.show();
  }); // cancel the overall process

  webcontent.on('cancel', function () {
    return close(webview);
  }); // process input, and trigger resize

  webcontent.on('submit', function (dimensions) {
    Object(_actions_resize__WEBPACK_IMPORTED_MODULE_2__["default"])(selection, dimensions);
    close(webview);
  }); // show the webview

  webview.loadURL(__webpack_require__(/*! ../resources/webview/webview.html */ "./resources/webview/webview.html"));
}; // close the window


var close = function close(webview) {
  webview.close();
  webview = null;
};
